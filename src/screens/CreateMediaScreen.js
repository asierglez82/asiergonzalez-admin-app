...
const mediaImage = require('../../assets/talk.png');
...
const CreateMediaScreen = ({ navigation }) => {
  const [imageUrl, setImageUrl] = useState('');
  const [language, setLanguage] = useState('es');
  const [notes, setNotes] = useState('');

  // Campos específicos de media (basados en Firebase)
  ...
  const generateUrl = (slug) => {
    if (!slug) return '';
    return `https://asiergonzalez.es/contenthub/media/${slug}`;
  };

  // Función para generar path automáticamente
  const generatePath = (slug) => {
    if (!slug) return '';
    return `/contenthub/media/${slug}`;
  };
  ...
  const handleGenerateContents = async () => {
    ...
    try {
      ...
      try {
        const prompt = `Genera contenido para un media/video con los siguientes datos:
...
{
  "description": "HTML completo de la descripción del media con formato (h6, p, ul, li, strong, em)",
  ...
  const generateLocalContent = () => {
    ...
    if (!description) {
      setDescription(`<h6>Media: <em>${title || '[Título del media]'}</em></h6><br><p>Descripción del media ${title || '[Título del media]'}</p>`);
    }
    if (!tags) {
      setTags('#media #video');
    }

    if (genLinkedin) setLinkedinText(`Media: "${title || '[Título del media]'}" - ${subtitle || '[Subtítulo]'}. #media #video`);
    if (genInstagram) setInstagramText(`"${title || '[Título del media]'}" - ${subtitle || '[Subtítulo]'} #media #video`);
    if (genTwitter) setTwitterText(`Media: "${title || '[Título del media]'}" - ${subtitle || '[Subtítulo]'}`);
    if (genWeb) setWebText(`Descubre mi media "${title || '[Título del media]'}"`);
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      // Crear el media como borrador INCLUYENDO campos de redes sociales
      const mediaData = {
        title,
        subtitle,
        image: finalImageUrl,
        event: `${slug}_media_view`,
        tags,
      ...
      // Crear el media en Firebase
      const createdMedia = await videosService.create(mediaData);
      ...
      // Navegar inmediatamente a la gestión de media
      navigation.navigate('MediaCRUD');
    } catch (error) {
      console.error(error);
      Alert.alert('Error', 'No se pudo guardar el borrador del media');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePublish = async () => {
    try {
      ...
      // Crear el media con todos los campos de Firebase incluyendo redes sociales
      const mediaData = {
        title,
        subtitle,
        image: finalImageUrl,
      ...
      // 1. PRIMERO: Crear el media en Firebase
      const createdMedia = await videosService.create(mediaData);
      console.log('Media creado en Firebase:', createdMedia);
      ...
      const webMessage = 'Media publicado en la web correctamente.';
      let socialMessage = '';
      ...
      Alert.alert(
        ...
        [{ text: 'OK', onPress: () => navigation.navigate('MediaCRUD') }]
      );
      
    } catch (error) {
      console.error(error);
      Alert.alert('Error', 'No se pudo crear el media');
    } finally {
      setSaving(false);
    }
  };
        ...
        <View style={styles.contentWrapper}>
          <View style={styles.header}>
            <View style={styles.headerContent}>
              <Text style={[styles.title, { color: '#00ca77' }]}>Nuevo Media</Text>
              <Text style={styles.subtitle}>Crear media con assets y contenido por red</Text>
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.grid}>
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Imagen del Media (URL)</Text>
                <TextInput
                  style={styles.input}
                  placeholder="https://..."
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Título del Media</Text>
                <TextInput
                  style={styles.input}
                  placeholder="Mi experiencia en Product Management"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={title}
            ...
            {imageUrl && (
              <View style={[styles.previewCard, isTablet && styles.previewCardTablet, isMobile && styles.previewCardMobile]}>
                {/* Preview del media - Composición: imagen del media con información */}
                {Platform.OS === 'web' ? (
                  <div 
                    ref={composeRef} 
                    style={{
                  ...
                  <ViewShot ref={composeRef} style={[styles.composeContainer, isTablet && styles.composeContainerTablet, isMobile && styles.composeContainerMobile]} options={{ format: 'png', quality: 1 }}>
                    <View style={[styles.composeBg, isTablet && styles.composeBgTablet, isMobile && styles.composeBgMobile]}>
                      ...
                      <Image 
                        source={mediaImage}
                        style={styles.backgroundMediaImage}
                        resizeMode="cover"
                      />
                      ...
                      <Image 
                        source={{ uri: imageUrl }} 
                        style={[styles.mediaImage, isTablet && styles.mediaImageTablet, isMobile && styles.mediaImageMobile]} 
                        resizeMode="contain" 
                      />
                      ...
                      {/* Subtítulo */}
                      {subtitle && (
                        <Text style={[styles.composeSubtitle, isTablet && styles.composeSubtitleTablet, isMobile && styles.composeSubtitleMobile]}>
                          {subtitle}
                        </Text>
                      )}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <View style={styles.labelRow}>
                  <Text style={styles.label}>Campos del Media</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="#productmanagement #media #video" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Link del video</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={url} 
                    onChangeText={setUrl} 
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="Contenido HTML para la descripción del media" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="/videos/productmanagementvideo.mp4" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="/contenthub/media/productmanagementmedia" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    value={url} 
                    onChangeText={setUrl} 
                    placeholder="https://asiergonzalez.es/contenthub/media/productmanagementmedia" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="product-management-media" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    style={styles.blogInput} 
                    value={`${slug}_media_view`} 
                    onChangeText={() => {}} 
                    placeholder="productmanagementmedia_media_view" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                </View>
