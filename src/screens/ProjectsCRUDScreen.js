...
const ProjectsCRUDScreen = ({ navigation }) => {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      const data = await projectsService.getAll('createdAt', 'desc', 100);
      setProjects(data);
    } catch (error) {
    ...
    <View style={styles.projectCard}>
      ...
      <View style={styles.projectContent}>
        <View style={styles.titleRow}>
          ...
          {(() => {
            const isPublished = project?.draft === false; // Publicado si draft === false
            return (
        ...
        <Text style={styles.projectBody} numberOfLines={2}>{stripHtml(project.description || '')}</Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalMessage}>¿Seguro que quieres eliminar este proyecto?</Text>
            {projectToDelete && (
              <View style={styles.modalPostInfo}>
                <Text style={styles.modalPostTitle} numberOfLines={2}>{projectToDelete.title || 'Sin título'}</Text>
                <Text style={styles.modalPostAuthor}>{projectToDelete.subtitle || 'Sin subtítulo'}</Text>
              </View>
            )}
