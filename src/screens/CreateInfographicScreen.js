...
const infographicImage = require('../../assets/infographic.png');
...
const CreateInfographicScreen = ({ navigation }) => {
  const [imageUrl, setImageUrl] = useState('');
  const [language, setLanguage] = useState('es');
  const [notes, setNotes] = useState('');

  // Campos específicos de infografías (basados en Firebase)
  ...
  const generateUrl = (slug) => {
    if (!slug) return '';
    return `https://asiergonzalez.es/contenthub/infographic/${slug}`;
  };

  // Función para generar path automáticamente
  const generatePath = (slug) => {
    if (!slug) return '';
    return `/contenthub/infographic/${slug}`;
  };
  ...
  const handleGenerateContents = async () => {
    ...
    try {
      ...
      try {
        const prompt = `Genera contenido para una infografía con los siguientes datos:
...
{
  "content": "HTML completo de la descripción de la infografía con formato (h6, p, ul, li, strong, em)",
  ...
  "platforms": {
    ...
    "twitter": "contenido para Twitter"
        ...
        try {
          ...
          // Campo category no existe en infografías, se omite
  ...
  const generateLocalContent = () => {
    ...
    if (!content) {
      setContent(`<h6>Infografía: <em>${title || '[Título de la infografía]'}</em></h6><br><p>Descripción de la infografía ${title || '[Título de la infografía]'} por ${author || '[Autor]'}</p>`);
    }
    if (!tags) {
      setTags('#infographic #visual');
    }

    if (genLinkedin) setLinkedinText(`Nueva infografía: "${title || '[Título de la infografía]'}" por ${author || '[Autor]'}. #infographic #visual`);
    if (genInstagram) setInstagramText(`"${title || '[Título de la infografía]'}" por ${author || '[Autor]'} #infographic #visual`);
    if (genTwitter) setTwitterText(`Infografía: "${title || '[Título de la infografía]'}" por ${author || '[Autor]'}`);
    if (genWeb) setWebText(`Descubre mi infografía "${title || '[Título de la infografía]'}"`);
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      // Crear el book en Firebase
        const createdInfographic = await infographicsService.create(infographicData);
      ...
      // Navegar inmediatamente a la gestión de books
        navigation.navigate('InfographicsCRUD');
    } catch (error) {
      console.error(error);
        Alert.alert('Error', 'No se pudo guardar el borrador de la infografía');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePublish = async () => {
    try {
      ...
      // 1. PRIMERO: Crear el book en Firebase
        const createdInfographic = await infographicsService.create(infographicData);
        console.log('Infografía creada en Firebase:', createdInfographic);
      ...
      // Mostrar mensaje de éxito
        const webMessage = 'Infografía publicada en la web correctamente.';
      let socialMessage = '';
      ...
      console.error(error);
        Alert.alert('Error', 'No se pudo crear la infografía');
    } finally {
      setSaving(false);
    }
  };
        ...
        <View style={styles.contentWrapper}>
          <View style={styles.header}>
            <View style={styles.headerContent}>
              <Text style={[styles.title, { color: '#00ca77' }]}>Nueva Infografía</Text>
              <Text style={styles.subtitle}>Crear infografía con assets y contenido por red</Text>
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.grid}>
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Imagen de la Infografía (URL)</Text>
                <TextInput
                  style={styles.input}
                  placeholder="https://..."
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Título de la Infografía</Text>
                <TextInput
                  style={styles.input}
                  placeholder="Guía Visual de Product Management"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={title}
              ...
              <View style={[styles.previewCard, isTablet && styles.previewCardTablet, isMobile && styles.previewCardMobile]}>
                ...
                {Platform.OS === 'web' ? (
                  ...
                  >
                    ...
                    <img 
                      src={getImageSource(infographicImage)}
                      style={{
                        position: 'absolute',
                        top: 'calc(50% + 30px)',
                        left: '50%',
                    ...
                    {/* Portada del libro centrada */}
                    <img 
                      src={imageDataUrl || imageUrl} 
                      style={{
                        width: isTablet ? '186px' : isMobile ? '126px' : '261px',
                        height: isTablet ? '267px' : isMobile ? '177px' : '352px',
                        maxHeight: isTablet ? '267px' : isMobile ? '177px' : '352px',
                        objectFit: 'cover',
                        borderRadius: '8px',
                    ...
                    {title && (
                      <div style={{
                        position: 'absolute',
                        top: isTablet ? '90px' : isMobile ? '80px' : '110px',
                        left: '50%',
                    ...
                    {author && (
                      <div style={{
                        position: 'absolute',
                        bottom: isTablet ? '215px' : isMobile ? '205px' : '235px',
                        left: '50%',
                        transform: 'translateX(-50%)',
                        color: '#333b4d',
                    ...
                    {/* Campo category eliminado para infografías */}
                  </div>
                ) : (
                  <ViewShot ref={composeRef} style={[styles.composeContainer, isTablet && styles.composeContainerTablet, isMobile && styles.composeContainerMobile]} options={{ format: 'png', quality: 1 }}>
                    <View style={[styles.composeBg, isTablet && styles.composeBgTablet, isMobile && styles.composeBgMobile]}>
                      {/* Imagen de cabecera (patrón CreatePostScreen) */}
                      <Image 
                        source={infographicImage}
                        style={styles.backgroundInfographicImage}
                        resizeMode="cover"
                      />
                      
                      {/* Imagen de la infografía centrada */}
                      <Image 
                        source={{ uri: imageUrl }} 
                        style={[styles.infographicImage, isTablet && styles.infographicImageTablet, isMobile && styles.infographicImageMobile]} 
                          resizeMode="cover"
                      />
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <View style={styles.labelRow}>
                  <Text style={styles.label}>Campos de la Infografía</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="#productmanagement #book #leadership" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Modal Image</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={modalImage} 
                    onChangeText={setModalImage} 
                    placeholder="/assets/infographics/modal-image.png" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="Contenido HTML para la reseña del libro" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="/assets/infographics/productmanagementguide.png" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                  {uploadedImagePath && (
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="/contenthub/infographic/productmanagementguide" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    value={url} 
                    onChangeText={setUrl} 
                    placeholder="https://asiergonzalez.es/contenthub/infographic/productmanagementguide" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="product-management-guide" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    style={styles.blogInput} 
                    value={`${slug}_infographic_view`} 
                    onChangeText={() => {}} 
                    placeholder="productmanagementguide_infographic_view" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                </View>
...
const styles = StyleSheet.create({
  ...
  infographicImageTablet: {
    width: 186,
    height: 267,
    maxHeight: 267,
  },
  infographicImageMobile: {
    width: 126,
    height: 177,
    maxHeight: 177,
  },
...
export default CreateInfographicScreen;
