...
const podcastImage = require('../../assets/podcast.png');
...
const CreatePodcastScreen = ({ navigation }) => {
  const [imageUrl, setImageUrl] = useState('');
  const [language, setLanguage] = useState('es');
  const [notes, setNotes] = useState('');

  // Campos específicos de podcast (basados en Firebase)
  ...
  const generateUrl = (slug) => {
    if (!slug) return '';
    return `https://asiergonzalez.es/contenthub/podcast/${slug}`;
  };

  // Función para generar path automáticamente
  const generatePath = (slug) => {
    if (!slug) return '';
    return `/contenthub/podcast/${slug}`;
  };
  ...
  const handleGenerateContents = async () => {
    ...
    try {
      ...
      try {
        const prompt = `Genera contenido para un podcast con los siguientes datos:
...
{
  "description": "HTML completo de la descripción del podcast con formato (h6, p, ul, li, strong, em)",
  ...
  const generateLocalContent = () => {
    ...
    if (!description) {
      setDescription(`<h6>Podcast: <em>${title || '[Título del podcast]'}</em></h6><br><p>Descripción del podcast ${title || '[Título del podcast]'}</p>`);
    }
    if (!tags) {
      setTags('#podcast #audio');
    }

    if (genLinkedin) setLinkedinText(`Podcast: "${title || '[Título del podcast]'}" - ${subtitle || '[Subtítulo]'}. #podcast #audio`);
    if (genInstagram) setInstagramText(`"${title || '[Título del podcast]'}" - ${subtitle || '[Subtítulo]'} #podcast #audio`);
    if (genTwitter) setTwitterText(`Podcast: "${title || '[Título del podcast]'}" - ${subtitle || '[Subtítulo]'}`);
    if (genWeb) setWebText(`Escucha mi podcast "${title || '[Título del podcast]'}"`);
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      const createdPodcast = await podcastService.create(podcastData);
      ...
      navigation.navigate('PodcastCRUD');
    } catch (error) {
      console.error(error);
      Alert.alert('Error', 'No se pudo guardar el borrador del podcast');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePublish = async () => {
    try {
      ...
      const createdPodcast = await podcastService.create(podcastData);
      console.log('Podcast creado en Firebase:', createdPodcast);
      ...
      const webMessage = 'Podcast publicado en la web correctamente.';
      let socialMessage = '';
      ...
      console.error(error);
      Alert.alert('Error', 'No se pudo crear el podcast');
    } finally {
      setSaving(false);
    }
  };
        ...
        <View style={styles.contentWrapper}>
          <View style={styles.header}>
            <View style={styles.headerContent}>
              <Text style={[styles.title, { color: '#00ca77' }]}>Nuevo Podcast</Text>
              <Text style={styles.subtitle}>Crear podcast con assets y contenido por red</Text>
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.grid}>
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Imagen del Podcast (URL)</Text>
                <TextInput
                  style={styles.input}
                  placeholder="https://..."
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Título del Podcast</Text>
                <TextInput
                  style={styles.input}
                  placeholder="El futuro del Product Management"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={title}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                ...
                <TextInput
                  style={styles.input}
                  placeholder="Conversación sobre tendencias y estrategias"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={subtitle}
            ...
            <View style={[styles.previewCard, isTablet && styles.previewCardTablet, isMobile && styles.previewCardMobile]}>
              {/* Preview del podcast - Composición: imagen del podcast con información */}
                {Platform.OS === 'web' ? (
                  <div 
                    ref={composeRef} 
                    style={{
                  ...
                  >
                    ...
                    <img 
                      src={getImageSource(podcastImage)}
                      style={{
                        position: 'absolute',
                        top: 'calc(50% + 30px)',
                        left: '50%',
                    ...
                    {/* Imagen del podcast centrada - solo si existe */}
                    ...
                    {subtitle && (
                      <div style={{
                        position: 'absolute',
                        top: (imageDataUrl || imageUrl) ? (isTablet ? '-60px' : isMobile ? '-70px' : '-40px') : (isTablet ? '80px' : isMobile ? '30px' : '180px'),
                        left: '50%',
                    ...
                    {title && (
                      <div style={{
                        position: 'absolute',
                        bottom: (imageDataUrl || imageUrl) ? (isTablet ? '355px' : isMobile ? '345px' : '375px') : (isTablet ? '230px' : isMobile ? '210px' : '260px'),
                        left: '50%',
                        transform: 'translateX(-50%)',
                        color: '#333b4d',
                  ...
                  <ViewShot ref={composeRef} style={[styles.composeContainer, isTablet && styles.composeContainerTablet, isMobile && styles.composeContainerMobile]} options={{ format: 'png', quality: 1 }}>
                    <View style={[styles.composeBg, isTablet && styles.composeBgTablet, isMobile && styles.composeBgMobile]}>
                      ...
                      <Image 
                        source={podcastImage}
                        style={styles.backgroundPodcastImage}
                        resizeMode="cover"
                      />
                      ...
                      {imageUrl && (
                        <Image 
                          source={{ uri: imageUrl }} 
                          style={[styles.podcastImage, isTablet && styles.podcastImageTablet, isMobile && styles.podcastImageMobile]} 
                          resizeMode="contain" 
                        />
                      )}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <View style={styles.labelRow}>
                  <Text style={styles.label}>Campos del Podcast</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="#productmanagement #podcast #audio" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Link del podcast</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={url} 
                    onChangeText={setUrl} 
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="Contenido HTML para la descripción del podcast" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Podcast Path</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={podcastpath} 
                    onChangeText={setPodcastpath} 
                    placeholder="/podcasts/productmanagementpodcast.mp3" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="/contenthub/podcast/productmanagementpodcast" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    value={url} 
                    onChangeText={setUrl} 
                    placeholder="https://asiergonzalez.es/contenthub/podcast/productmanagementpodcast" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="product-management-podcast" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    style={styles.blogInput} 
                    value={`${slug}_podcast_view`} 
                    onChangeText={() => {}} 
                    placeholder="productmanagementpodcast_podcast_view" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                </View>
