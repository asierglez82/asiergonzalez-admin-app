...
const conferenceImage = require('../../assets/conference.png');
...
const CreateConferenceScreen = ({ navigation }) => {
  ...
  // Campos específicos de conferencia (basados en Firebase)
  ...
  const generateUrl = (slug) => {
    if (!slug) return '';
    return `https://asiergonzalez.es/contenthub/conference/${slug}`;
  };

  // Función para generar path automáticamente
  const generatePath = (slug) => {
    if (!slug) return '';
    return `/contenthub/conference/${slug}`;
  };
  ...
  const handleGenerateContents = async () => {
    ...
    try {
      ...
      try {
        const prompt = `Genera contenido para una conferencia con los siguientes datos:
...
IMPORTANTE: 
- Los campos título y subtítulo son fijos y NO deben ser generados ni modificados.
- NO uses emojis en ninguna de las respuestas.

Devuelve un JSON con estos campos:
{
  "description": "HTML completo de la descripción de la conferencia con formato (h6, p, ul, li, strong, em)",
    ...
    "twitter": "contenido para Twitter"
        ...
        try {
          ...
          if (parsed.description) {
            ...
            generatedFields.add('description');
          }
  ...
  const generateLocalContent = () => {
    ...
    // NO generamos título ni subtítulo ya que son campos fijos
    if (!description) {
      setDescription(`<h6>Conferencia: <em>${title || '[Título de la conferencia]'}</em></h6><br><p>Descripción de la conferencia ${title || '[Título de la conferencia]'}</p>`);
    }
    if (!tags) {
      setTags('#conference #speaking');
    }

    if (genLinkedin) setLinkedinText(`Conferencia: "${title || '[Título de la conferencia]'}" - ${subtitle || '[Subtítulo]'}. #conference #speaking`);
    if (genInstagram) setInstagramText(`"${title || '[Título de la conferencia]'}" - ${subtitle || '[Subtítulo]'} #conference #speaking`);
    if (genTwitter) setTwitterText(`Conferencia: "${title || '[Título de la conferencia]'}" - ${subtitle || '[Subtítulo]'}`);
    if (genWeb) setWebText(`Descubre mi conferencia "${title || '[Título de la conferencia]'}"`);
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      // Crear el podcast como borrador INCLUYENDO campos de redes sociales
        const conferenceData = {
        title,
        subtitle,
        image: finalImageUrl,
        event: `${slug}_conference_view`,
        tags,
      ...
      // Crear el podcast en Firebase
        const createdConference = await conferencesService.create(conferenceData);
      ...
      // Navegar inmediatamente a la gestión de podcasts
        navigation.navigate('ConferencesCRUD');
    } catch (error) {
      console.error(error);
        Alert.alert('Error', 'No se pudo guardar el borrador de la conferencia');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePublish = async () => {
    try {
      ...
      // Crear el podcast con todos los campos de Firebase incluyendo redes sociales
        const conferenceData = {
        title,
        subtitle,
        image: finalImageUrl,
      ...
      // 1. PRIMERO: Crear el podcast en Firebase
        const createdConference = await conferencesService.create(conferenceData);
        console.log('Conferencia creada en Firebase:', createdConference);
      ...
      // Mostrar mensaje de éxito
        const webMessage = 'Conferencia publicada en la web correctamente.';
      let socialMessage = '';
      ...
      Alert.alert(
        ...
        [{ text: 'OK', onPress: () => navigation.navigate('PodcastCRUD') }]
      );
      
    } catch (error) {
      console.error(error);
        Alert.alert('Error', 'No se pudo crear la conferencia');
    } finally {
      setSaving(false);
    }
  };
        ...
        <View style={styles.contentWrapper}>
          <View style={styles.header}>
            <View style={styles.headerContent}>
              <Text style={[styles.title, { color: '#00ca77' }]}>Nueva Conferencia</Text>
              <Text style={styles.subtitle}>Crear conferencia con assets y contenido por red</Text>
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.grid}>
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Imagen de la Conferencia (URL)</Text>
                <TextInput
                  style={styles.input}
                  placeholder="https://..."
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Título de la Conferencia</Text>
                <TextInput
                  style={styles.input}
                  placeholder="Innovación en Product Management 2024"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={title}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                ...
                <TextInput
                  style={styles.input}
                  placeholder="Tendencias y estrategias para el futuro del producto"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={subtitle}
            ...
            <View style={[styles.previewCard, isTablet && styles.previewCardTablet, isMobile && styles.previewCardMobile]}>
              {/* Preview de la conferencia - Composición: imagen de la conferencia con información */}
                {Platform.OS === 'web' ? (
                  <div 
                    ref={composeRef} 
                    style={{
                  ...
                  >
                    ...
                    <img 
                      src={getImageSource(conferenceImage)}
                      style={{
                        position: 'absolute',
                        top: 'calc(50% + 30px)',
                        left: '50%',
                    ...
                    {/* Imagen de la conferencia centrada - solo si existe */}
                    ...
                    {title && (
                      <div style={{
                        position: 'absolute',
                        top: (imageDataUrl || imageUrl) ? (isTablet ? '340px' : isMobile ? '330px' : '360px') : (isTablet ? '480px' : isMobile ? '430px' : '580px'),
                        left: '50%',
                    ...
                    {subtitle && (
                      <div style={{
                        position: 'absolute',
                        bottom: (imageDataUrl || imageUrl) ? (isTablet ? '385px' : isMobile ? '375px' : '405px') : (isTablet ? '260px' : isMobile ? '240px' : '290px'),
                        left: '50%',
                        transform: 'translateX(-50%)',
                        color: '#333b4d',
                  ...
                  <ViewShot ref={composeRef} style={[styles.composeContainer, isTablet && styles.composeContainerTablet, isMobile && styles.composeContainerMobile]} options={{ format: 'png', quality: 1 }}>
                    <View style={[styles.composeBg, isTablet && styles.composeBgTablet, isMobile && styles.composeBgMobile]}>
                      {/* Imagen de cabecera (patrón CreatePostScreen) */}
                      <Image 
                        source={conferenceImage}
                        style={styles.backgroundConferenceImage}
                        resizeMode="cover"
                      />
                      ...
                      {imageUrl && (
                        <Image 
                          source={{ uri: imageUrl }} 
                          style={[styles.conferenceImage, isTablet && styles.conferenceImageTablet, isMobile && styles.conferenceImageMobile]} 
                          resizeMode="contain" 
                        />
                      )}
                      ...
                      {title && (
                        <Text style={[
                          styles.composeTitle, 
                          isTablet && styles.composeTitleTablet, 
                          isMobile && styles.composeTitleMobile,
                          !imageUrl && styles.composeTitleNoImage
                        ]}>
                          {title}
                        </Text>
                      )}
                      ...
                      {subtitle && (
                        <Text style={[
                          styles.composeSubtitle, 
                          isTablet && styles.composeSubtitleTablet, 
                          isMobile && styles.composeSubtitleMobile,
                          !imageUrl && styles.composeSubtitleNoImage
                        ]}>
                          {subtitle}
                        </Text>
                      )}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <View style={styles.labelRow}>
                  <Text style={styles.label}>Campos de la Conferencia</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.fieldHeader}>
                    <Text style={styles.blogFieldLabel}>Subtítulo *</Text>
                    {aiGeneratedFields.has('subtitle') && (
                      <View style={styles.aiGeneratedBadge}>
                        <Ionicons name="sparkles" size={12} color="#4CAF50" />
                        <Text style={styles.aiGeneratedText}>IA</Text>
                      </View>
                  ...
                  <TextInput 
                    ...
                    value={subtitle} 
                    onChangeText={setSubtitle} 
                    placeholder="Una reflexión sobre el futuro del Product Management" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="#productmanagement #conference #speaking" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Link de la conferencia</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={url} 
                    onChangeText={setUrl} 
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.fieldHeader}>
                    <Text style={styles.blogFieldLabel}>Descripción (HTML)</Text>
                    {aiGeneratedFields.has('description') && (
                      <View style={styles.aiGeneratedBadge}>
                        <Ionicons name="sparkles" size={12} color="#4CAF50" />
                        <Text style={styles.aiGeneratedText}>IA</Text>
                      </View>
                  ...
                  <TextInput 
                    ...
                    placeholder="Contenido HTML para la descripción de la conferencia" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Video Path</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={videopath} 
                    onChangeText={setVideopath} 
                    placeholder="/videos/productmanagementconference.mp4" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="/contenthub/conference/productmanagementconference" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    value={url} 
                    onChangeText={setUrl} 
                    placeholder="https://asiergonzalez.es/contenthub/conference/productmanagementconference" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    ...
                    placeholder="product-management-conference" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  ...
                  <TextInput 
                    style={styles.blogInput} 
                    value={`${slug}_conference_view`} 
                    onChangeText={() => {}} 
                    placeholder="productmanagementconference_conference_view" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                </View>
...
const styles = StyleSheet.create({
  ...
  conferenceImageTablet: {
    width: 400,
    height: 225,
    marginTop: -250,
  },
  conferenceImageMobile: {
    width: 280,
    height: 157,
    marginTop: -230,
  },
  ...
  composeSubtitleTablet: {
    fontSize: 20,
    bottom: 245,
  },
  composeSubtitleMobile: {
    fontSize: 16,
    bottom: 235,
  },
  composeTitleNoImage: {
