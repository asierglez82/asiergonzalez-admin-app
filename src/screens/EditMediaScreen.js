...
const EditMediaScreen = ({ navigation, route }) => {
  ...
  const generateSocialMediaContent = async () => {
    try {
      ...
      const prompt = `Genera contenido para redes sociales basado en este video/media:
...
LinkedIn: Contenido profesional y detallado sobre el video/media (máximo 3000 caracteres)
Instagram: Contenido visual y atractivo sobre el video/media (máximo 2200 caracteres)  
Twitter: Contenido conciso y directo sobre el video/media (máximo 280 caracteres)
...
Devuelve un JSON con esta estructura:
{
  ...
  const loadMedia = async () => {
    try {
      setLoading(true);
      const data = await videosService.getById(mediaId);
      setMedia(data);
      setFormData({
        title: data.title || '',
      ...
      Alert.alert('Error', 'No se pudo cargar el media');
      navigation.goBack();
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (mediaId) loadMedia(); else { Alert.alert('Error', 'Falta ID'); navigation.goBack(); }
  }, [mediaId]);

  const handleInputChange = (field, value) => setFormData(prev => ({ ...prev, [field]: value }));

  const handleBack = () => navigation.navigate('MediaCRUD');

  // Asegurar URL pública de imagen: si no es http(s), subir a Storage (media-images)
  const ensurePublicImageUrl = async (imageUrl) => {
    try {
      if (!imageUrl) return '';
      if (/^https?:\/\//i.test(imageUrl)) return imageUrl;
      const upload = await storageService.uploadImage(imageUrl, 'media-images');
  ...
  const handleCancel = () => {
    ...
    if (media.socialMedia) {
      setSocialMediaData({
        linkedin: media.socialMedia.linkedin || '',
        instagram: media.socialMedia.instagram || '',
        twitter: media.socialMedia.twitter || '',
        settings: {
          genLinkedin: media.socialMedia.settings?.genLinkedin !== false,
          genInstagram: media.socialMedia.settings?.genInstagram !== false,
          genTwitter: media.socialMedia.settings?.genTwitter || false,
        },
      });
      setPublishedStatus({
        linkedin: media.socialMedia?.published?.linkedin === true,
        instagram: media.socialMedia?.published?.instagram === true,
        twitter: media.socialMedia?.published?.twitter === true,
      });
    }
    Alert.alert('Cambios deshechos', 'Se han restaurado los valores originales de el media');
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await videosService.update(mediaId, updateData);
      Alert.alert('Éxito', 'Media actualizado', [{ text: 'OK', onPress: () => navigation.navigate('MediaCRUD') }]);
    } catch (e) {
      Alert.alert('Error', 'No se pudo actualizar el media');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePublish = async () => {
    try {
      ...
      Alert.alert('Publicación completada', 'Quote publicada correctamente' + (result?.success ? ' y publicada en redes' : ''), [{ text: 'OK', onPress: () => navigation.navigate('MediaCRUD') }]);
    } catch (e) {
      Alert.alert('Error', 'No se pudo publicar el media');
    } finally {
      setSaving(false);
    }
  };
    ...
    <View style={styles.container}>
      <View style={[styles.header, isTablet && styles.headerTablet, isMobile && styles.headerMobile]}>
        <View style={[styles.headerContent, isTablet && styles.headerContentTablet, isMobile && styles.headerContentMobile]}>
          <Text style={[styles.title, isTablet && styles.titleTablet, isMobile && styles.titleMobile]}>Editar Media</Text>
          <Text style={[styles.subtitle, isTablet && styles.subtitleTablet, isMobile && styles.subtitleMobile]}>Modifica los campos de el media</Text>
        </View>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, isTablet && styles.scrollContentTablet, isMobile && styles.scrollContentMobile]}>
        <View style={[styles.section, isTablet && styles.sectionTablet, isMobile && styles.sectionMobile]}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.title} onChangeText={(v)=>handleInputChange('title', v)} placeholder="Título del video" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Subtítulo</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.subtitle} onChangeText={(v)=>handleInputChange('subtitle', v)} placeholder="Subtítulo del video" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Fecha</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.date} onChangeText={(v)=>handleInputChange('date', v)} placeholder="Ej: 2017" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Tags</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.tags} onChangeText={(v)=>handleInputChange('tags', v)} placeholder="Ej: #mmd17 #iot" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Evento</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.event} onChangeText={(v)=>handleInputChange('event', v)} placeholder="Ej: iot_teaser_video_view" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>
        ...
        <View style={[styles.section, isTablet && styles.sectionTablet, isMobile && styles.sectionMobile]}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={[styles.textArea, isTablet && styles.textAreaTablet, isMobile && styles.textAreaMobile]} value={formData.description} onChangeText={(v)=>handleInputChange('description', v)} placeholder="Descripción del video" placeholderTextColor="rgba(255,255,255,0.5)" multiline numberOfLines={4} />
          </View>
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Imagen</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.image} onChangeText={(v)=>handleInputChange('image', v)} placeholder="URL imagen del video" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>
          <View style={styles.inputGroup}>
