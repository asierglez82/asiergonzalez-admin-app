...
const EditPodcastScreen = ({ navigation, route }) => {
  ...
  const generateSocialMediaContent = async () => {
    try {
      ...
      const prompt = `Genera contenido para redes sociales basado en este episodio de podcast:
...
LinkedIn: Contenido profesional y detallado sobre el episodio del podcast (máximo 3000 caracteres)
Instagram: Contenido visual y atractivo sobre el episodio del podcast (máximo 2200 caracteres)  
Twitter: Contenido conciso y directo sobre el episodio del podcast (máximo 280 caracteres)
...
Devuelve un JSON con esta estructura:
{
  ...
  const loadPodcast = async () => {
    try {
      ...
      Alert.alert('Error', 'No se pudo cargar el podcast');
      navigation.goBack();
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (podcastId) loadPodcast(); else { Alert.alert('Error', 'Falta ID'); navigation.goBack(); }
  }, [podcastId]);

  const handleInputChange = (field, value) => setFormData(prev => ({ ...prev, [field]: value }));

  const handleBack = () => navigation.navigate('PodcastCRUD');

  // Asegurar URL pública de imagen: si no es http(s), subir a Storage (podcast-images)
  const ensurePublicImageUrl = async (imageUrl) => {
    try {
      if (!imageUrl) return '';
      if (/^https?:\/\//i.test(imageUrl)) return imageUrl;
      const upload = await storageService.uploadImage(imageUrl, 'podcast-images');
  ...
  const handleCancel = () => {
    ...
    if (podcast.socialMedia) {
      setSocialMediaData({
        linkedin: podcast.socialMedia.linkedin || '',
        instagram: podcast.socialMedia.instagram || '',
        twitter: podcast.socialMedia.twitter || '',
        settings: {
          genLinkedin: podcast.socialMedia.settings?.genLinkedin !== false,
          genInstagram: podcast.socialMedia.settings?.genInstagram !== false,
          genTwitter: podcast.socialMedia.settings?.genTwitter || false,
        },
      });
      setPublishedStatus({
        linkedin: podcast.socialMedia?.published?.linkedin === true,
        instagram: podcast.socialMedia?.published?.instagram === true,
        twitter: podcast.socialMedia?.published?.twitter === true,
      });
    }
    Alert.alert('Cambios deshechos', 'Se han restaurado los valores originales de el podcast');
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await podcastService.update(podcastId, updateData);
      Alert.alert('Éxito', 'Podcast actualizado', [{ text: 'OK', onPress: () => navigation.navigate('PodcastCRUD') }]);
    } catch (e) {
      Alert.alert('Error', 'No se pudo actualizar el podcast');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePublish = async () => {
    try {
      ...
      Alert.alert('Publicación completada', 'Quote publicada correctamente' + (result?.success ? ' y publicada en redes' : ''), [{ text: 'OK', onPress: () => navigation.navigate('PodcastCRUD') }]);
    } catch (e) {
      Alert.alert('Error', 'No se pudo publicar el podcast');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Cargando podcast...</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      <View style={[styles.header, isTablet && styles.headerTablet, isMobile && styles.headerMobile]}>
        <View style={[styles.headerContent, isTablet && styles.headerContentTablet, isMobile && styles.headerContentMobile]}>
          <Text style={[styles.title, isTablet && styles.titleTablet, isMobile && styles.titleMobile]}>Editar Podcast</Text>
          <Text style={[styles.subtitle, isTablet && styles.subtitleTablet, isMobile && styles.subtitleMobile]}>Modifica los campos de el podcast</Text>
        </View>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, isTablet && styles.scrollContentTablet, isMobile && styles.scrollContentMobile]}>
        <View style={[styles.section, isTablet && styles.sectionTablet, isMobile && styles.sectionMobile]}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.title} onChangeText={(v)=>handleInputChange('title', v)} placeholder="Título del episodio" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Subtítulo</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.subtitle} onChangeText={(v)=>handleInputChange('subtitle', v)} placeholder="Subtítulo del episodio" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Fecha</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.date} onChangeText={(v)=>handleInputChange('date', v)} placeholder="Ej: 21 February 2025" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Tags</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.tags} onChangeText={(v)=>handleInputChange('tags', v)} placeholder="Ej: #amazon #innovation" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>

          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Evento</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.event} onChangeText={(v)=>handleInputChange('event', v)} placeholder="Ej: podcast_view" placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>
        ...
        <View style={[styles.section, isTablet && styles.sectionTablet, isMobile && styles.sectionMobile]}>
          <Text style={[styles.sectionTitle, isTablet && styles.sectionTitleTablet, isMobile && styles.sectionTitleMobile]}>Contenido y Podcast</Text>
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Descripción</Text>
            <TextInput style={[styles.textArea, isTablet && styles.textAreaTablet, isMobile && styles.textAreaMobile]} value={formData.description} onChangeText={(v)=>handleInputChange('description', v)} placeholder="Descripción del episodio" placeholderTextColor="rgba(255,255,255,0.5)" multiline numberOfLines={4} />
          </View>
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isTablet && styles.labelTablet, isMobile && styles.labelMobile]}>Podcast Path (Spotify Embed)</Text>
            <TextInput style={[styles.input, isTablet && styles.inputTablet, isMobile && styles.inputMobile]} value={formData.podcastpath} onChangeText={(v)=>handleInputChange('podcastpath', v)} placeholder="https://open.spotify.com/embed/..." placeholderTextColor="rgba(255,255,255,0.5)" />
          </View>
          <View style={styles.cardAccent} />
        </View>
...
export default EditPodcastScreen;
