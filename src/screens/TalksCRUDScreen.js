...
const TalksCRUDScreen = ({ navigation }) => {
  ...
  const loadTalks = async () => {
    try {
      setLoading(true);
      const data = await talksService.getAll('createdAt', 'desc', 100);
      setTalks(data);
    } catch (error) {
  ...
  const handleDelete = (talkId) => {
    const t = talks.find(t => t.id === talkId);
    setTalkToDelete(t);
  ...
  const confirmDelete = async () => {
    ...
    await deleteTalk(talkToDelete.id);
    ...
    <View style={styles.talkCard}>
      ...
      <View style={styles.talkContent}>
        <View style={styles.titleRow}>
          <Text style={styles.talkTitle} numberOfLines={1}>{talk.title}</Text>
          {(() => {
            const isPublished = talk?.draft === false; // Publicado si draft === false
            return (
        ...
        <Text style={styles.talkBody} numberOfLines={2}>{stripHtml(talk.content || talk.description || '')}</Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalMessage}>¿Seguro que quieres eliminar este talk?</Text>
            {talkToDelete && (
              <View style={styles.modalPostInfo}>
                <Text style={styles.modalPostTitle} numberOfLines={2}>{talkToDelete.title || 'Sin título'}</Text>
                <Text style={styles.modalPostAuthor}>{talkToDelete.subtitle || 'Sin subtítulo'}</Text>
              </View>
            )}
