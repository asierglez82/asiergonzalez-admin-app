...
const InfographicsCRUDScreen = ({ navigation }) => {
  ...
  const loadInfographics = async () => {
    try {
      setLoading(true);
      const data = await infographicsService.getAll('createdAt', 'desc', 100);
  ...
  const handleDelete = (infographicId) => {
    const i = infographics.find(i => i.id === infographicId);
    setInfographicToDelete(i);
    setShowDeleteModal(true);
  };

  const confirmDelete = async () => {
    if (!infographicToDelete) return;
    await deleteInfographic(infographicToDelete.id);
    ...
    <View style={styles.infographicCard}>
      ...
      <View style={styles.infographicContent}>
        <View style={styles.titleRow}>
          <Text style={styles.infographicTitle} numberOfLines={1}>{infographic.title}</Text>
          {(() => {
            const isPublished = infographic?.draft === false; // Publicado si draft === false
            return (
        ...
        <Text style={styles.infographicBody} numberOfLines={2}>{stripHtml(infographic.description || '')}</Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalMessage}>¿Seguro que quieres eliminar esta infografía?</Text>
            {infographicToDelete && (
              <View style={styles.modalPostInfo}>
                <Text style={styles.modalPostTitle} numberOfLines={2}>{infographicToDelete.title || 'Sin título'}</Text>
                <Text style={styles.modalPostAuthor}>{infographicToDelete.subtitle || 'Sin subtítulo'}</Text>
              </View>
            )}
