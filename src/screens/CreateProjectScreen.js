...
const CreateProjectScreen = ({ navigation }) => {
  ...
  // Campos específicos de proyectos (basados en Firebase)
  ...
  const generateUrl = (slug) => {
    if (!slug) return '';
    return `https://asiergonzalez.es/contenthub/project/${slug}`;
  };

  // Función para generar path automáticamente
  const generatePath = (slug) => {
    if (!slug) return '';
    return `/contenthub/project/${slug}`;
  };
  ...
  const handleGenerateContents = async () => {
    ...
    try {
      ...
      try {
        const prompt = `Genera contenido para una reseña de libro con los siguientes datos:
...
IMPORTANTE: 
- Los campos título y autor son fijos y NO deben ser generados ni modificados. La categoría SÍ debe ser generada por la IA.
- NO uses emojis en ninguna de las respuestas.

Devuelve un JSON con estos campos:
{
  "category": "categoría del libro (ej: Product Management, Fiction, Business, etc.)",
  "content": "HTML completo de la reseña del libro con formato (h6, p, ul, li, strong, em)",
  ...
  const generateLocalContent = () => {
    ...
    // NO generamos título ni autor ya que son campos fijos
    // La categoría se genera por IA, no aquí
    if (!content) {
      setContent(`<h6>Reseña: <em>${title || '[Título del libro]'}</em></h6><br><p>Reseña del libro ${title || '[Título del libro]'} por ${author || '[Autor]'}</p>`);
    }
    if (!tags) {
      setTags('#book #reading');
    }

    if (genLinkedin) setLinkedinText(`Acabo de leer "${title || '[Título del libro]'}" de ${author || '[Autor]'}. #book #reading`);
    if (genInstagram) setInstagramText(`"${title || '[Título del libro]'}" por ${author || '[Autor]'} #book #reading`);
    if (genTwitter) setTwitterText(`Recomiendo "${title || '[Título del libro]'}" de ${author || '[Autor]'}`);
    if (genWeb) setWebText(`Descubre mi reseña de "${title || '[Título del libro]'}"`);
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      // Crear el book como borrador INCLUYENDO campos de redes sociales
      const bookData = {
        author,
        buy,
        category,
        content,
        event: `${slug}_book_view`,
      ...
      // Navegar inmediatamente a la gestión de books
      navigation.navigate('BooksCRUD');
    } catch (error) {
      console.error(error);
  ...
  const handlePublish = async () => {
    try {
      ...
      // Crear el book con todos los campos de Firebase incluyendo redes sociales
      const bookData = {
        author,
        buy,
        category,
      ...
      // 1. PRIMERO: Crear el book en Firebase
      const createdBook = await booksService.create(bookData);
      console.log('Book creado en Firebase:', createdBook);
      ...
      const webMessage = 'Book publicado en la web correctamente.';
      let socialMessage = '';
      ...
      Alert.alert(
        ...
        [{ text: 'OK', onPress: () => navigation.navigate('BooksCRUD') }]
      );
      
    } catch (error) {
      console.error(error);
      Alert.alert('Error', 'No se pudo crear el book');
    } finally {
      setSaving(false);
    }
  };
        ...
        <View style={styles.contentWrapper}>
          <View style={styles.header}>
            <View style={styles.headerContent}>
              <Text style={[styles.title, { color: '#00ca77' }]}>Nuevo Proyecto</Text>
              <Text style={styles.subtitle}>Crear proyecto con assets y contenido por red</Text>
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.grid}>
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Imagen del Proyecto (URL)</Text>
                <TextInput
                  style={styles.input}
                  placeholder="https://..."
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Idioma del Proyecto</Text>
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                ...
                <TextInput
                  style={styles.input}
                  placeholder="Mi Proyecto de Product Management"
                  placeholderTextColor="rgba(255,255,255,0.4)"
                  value={title}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Autor del Proyecto</Text>
                <TextInput
                  style={styles.input}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <Text style={styles.label}>Notas del Proyecto</Text>
                <TextInput style={[styles.input, styles.multiline]} multiline value={notes} onChangeText={setNotes} placeholder="Notas internas sobre el proyecto (no públicas)" placeholderTextColor="rgba(255,255,255,0.4)" />
              </View>
              <View style={[styles.cardAccent, { backgroundColor: '#00ca77' }]} />
            </View>
            ...
            {(imageDataUrl || imageUrl) && (
              <View style={[styles.previewCard, isTablet && styles.previewCardTablet, isMobile && styles.previewCardMobile]}>
                {/* Preview de la portada del libro - Composición: portada del libro con información */}
                {Platform.OS === 'web' ? (
                  <div 
                    ref={composeRef} 
                    style={{
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      backgroundColor: '#00ca77',
                      padding: isTablet ? '40px 60px' : isMobile ? '30px 50px' : '50px 80px',
                      borderRadius: '12px',
                      boxShadow: '0 15px 40px rgba(0,0,0,0.4)',
                      zIndex: 2,
                      minWidth: isTablet ? '300px' : isMobile ? '250px' : '400px',
                      textAlign: 'center'
                    }}>
                      {title && (
                        <div style={{
                          color: '#ffffff',
                          fontSize: isTablet ? '32px' : isMobile ? '24px' : '48px',
                          fontWeight: '700',
                  ...
                  <ViewShot ref={composeRef} style={[styles.composeContainer, isTablet && styles.composeContainerTablet, isMobile && styles.composeContainerMobile]} options={{ format: 'png', quality: 1 }}>
                    <View style={[styles.composeBg, isTablet && styles.composeBgTablet, isMobile && styles.composeBgMobile]}>
                      {/* Imagen de fondo que ocupa todo el espacio */}
                      <Image 
                        source={{ uri: imageDataUrl || imageUrl }}
                        style={styles.backgroundProjectImage}
                        resizeMode="cover"
                      />
                      ...
                      <View style={styles.greenBoxContainer}>
                        {title && (
                          <Text style={[styles.greenBoxTitle, isTablet && styles.greenBoxTitleTablet, isMobile && styles.greenBoxTitleMobile]}>
                            {title}
                          </Text>
                        )}
                      </View>
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.fieldContent}>
                <View style={styles.labelRow}>
                  <Text style={styles.label}>Campos del Book</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.fieldHeader}>
                    <Text style={styles.blogFieldLabel}>Título del Proyecto *</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.fieldHeader}>
                    <Text style={styles.blogFieldLabel}>Autor del Proyecto *</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.fieldHeader}>
                    <Text style={styles.blogFieldLabel}>Categoría del Proyecto *</Text>
                ...
                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Tags del Proyecto</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={tags} 
                    onChangeText={setTags} 
                    placeholder="#productmanagement #project #development" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Link del Proyecto</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={buy} 
                    onChangeText={setBuy} 
                    placeholder="https://github.com/asiergonzalez/proyecto" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.fieldHeader}>
                    <Text style={styles.blogFieldLabel}>Descripción del Proyecto (HTML)</Text>
                  ...
                  <TextInput 
                    ...
                    placeholder="Contenido HTML para la descripción del proyecto" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  <View style={styles.imageFieldContainer}>
                    <Text style={styles.blogFieldLabel}>Imagen del Proyecto</Text>
                    {uploadedImagePath && (
                      <TouchableOpacity 
                  ...
                  <TextInput 
                    ...
                    placeholder="/assets/projects/miprojecto.png" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                  {uploadedImagePath && (
                ...
                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Path del Proyecto</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={path} 
                    onChangeText={setPath} 
                    placeholder="/contenthub/project/miprojecto" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>URL del Proyecto</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={url} 
                    onChangeText={setUrl} 
                    placeholder="https://asiergonzalez.es/contenthub/project/miprojecto" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>

                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Slug del Proyecto</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={slug} 
                    onChangeText={setSlug} 
                    placeholder="mi-proyecto-product-management" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                  />
                </View>
                ...
                <View style={styles.blogFieldGroup}>
                  <Text style={styles.blogFieldLabel}>Event del Proyecto</Text>
                  <TextInput 
                    style={styles.blogInput} 
                    value={`${slug}_project_view`} 
                    onChangeText={() => {}} 
                    placeholder="miprojecto_project_view" 
                    placeholderTextColor="rgba(255,255,255,0.4)" 
                    editable={false}
                  />
                </View>
